...
export function generateStaticParams() {
  return SUPPORTED_LOCALES.flatMap((locale) => CONVERT_SLUGS.map((slug) => ({ locale, slug })));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale, slug } = await params;
  if (!isSupportedLocale(locale) || !CONVERT_SLUGS.includes(slug as ConvertSlug)) return {};
  const seo = getConvertSeo(slug as ConvertSlug);
  const languages = Object.fromEntries(
    SUPPORTED_LOCALES.map((l) => [l, localizedPath(l, `/convert/${slug}`)])
  );
  return {
    title: seo.title,
    description: seo.description,
    alternates: {
      canonical: localizedPath(locale, `/convert/${slug}`),
      languages: {
        ...languages,
    ...
    openGraph: {
      title: seo.title,
      description: seo.description,
      url: localizedPath(locale, `/convert/${slug}`),
...
export default async function LocalizedConvertPage({ params }: Props) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Convert", item: `${base}${localizedPath(safeLocale, "/convert/avif-to-jpg")}` },
  ...
  return (
    <>
      ...
      <IntroCard title={t.whyConvert} content={seo.whyConvert} />
      <IntroCard title={t.howItWorks} content={seo.howItWorks} />
      ...
      <FAQ items={seo.faq} heading={t.faqTitle} />
      <InternalLinks currentSlug={slug} locale={safeLocale} />
    </>
  );
}
